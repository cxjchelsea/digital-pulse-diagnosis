...
> 当前状态：D0、D1、D2已验收并合并；D3数字阶段已完成并验收；下一阶段进入H1真实硬件采购、到货验收和手动单点台架验证。本项目输出客观脉搏参数和工程验证结果，不输出脉象、证候、疾病或治疗结论。
...
7. [D3阶段设计与实现详解](docs/00-项目总览/D3阶段设计与实现详解.md)：理解自动施压与安全仿真；
8. [D3自动施压与安全控制数字仿真实施方案](docs/00-项目总览/D3自动施压与安全控制数字仿真实施方案.md)：评审控制、安全和故障验收基线；
9. 进入当前阶段对应目录，不需要一次读完所有文档。

当前状态是D0至D2已完成；D3-A至D3-G及收尾强化已完成：正常Profile、完整故障卸载、ABORT API/Web闭环、1800秒完整链路、故障矩阵、API重放、Web构建、验收证据生成与GitHub Actions均已落地。合并并确认CI通过后进入H1准备：
