...
> 当前状态：D0 合成环境闭环已通过并合并；当前进入 D1–D3 购买前数字原型扩展，H1 真实硬件采购与调试等待数字阶段通过。本项目输出客观脉搏参数和工程验证结果，不输出脉象、证候、疾病或治疗结论。
...
6. 进入当前阶段对应目录，不需要一次读完所有文档。

当前状态是D1功能已实现、等待依赖完整环境的全量API测试与合并前验收；尚未进入D2。完成D1–D3验收后，再执行：
