...
function App(){
 const [health,setHealth]=useState('检查中'); const [result,setResult]=useState<Result|null>(null); const [busy,setBusy]=useState(false); const [error,setError]=useState(''); const [wave,setWave]=useState<number[]>([]); const [force,setForce]=useState<number[]>([]);
 useEffect(()=>{fetch('/api/health').then(r=>r.json()).then(()=>setHealth('P0服务正常')).catch(()=>setHealth('服务未连接'));},[]);
 ...
 return <main><header><div><p className="eyebrow">DIGITAL PULSE · P0</p><h1>脉搏采集软件闭环</h1><p>模拟采集 → 质量门控 → 多压力分析</p></div><span className="status">{health}</span></header>
